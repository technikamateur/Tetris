    ...
    for i in {1..5}; do
        for core in ${cores[@]}; do
            for thr in ${threads[@]}; do
                perf stat --field-separator , -e user_time,system_time,duration_time,energy-pkg,energy-cores env LD_PRELOAD=./is_it_openmp.so $f 2>>$bname#$core,$thr.txt | tee -a $bname.log &
